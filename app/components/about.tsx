      ...
      {open ? (
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="section-inner max-w-3xl py-10 sm:py-12 md:py-16">
              <p className="pr-12 text-sm font-semibold text-gold-dim">{about.eyebrow}</p>
              <h3
                id="about-detail-title"
                className="mt-2 font-display text-3xl font-bold uppercase tracking-tight text-foreground sm:text-4xl"
              >
                {about.title}
              </h3>

              <div className="mt-6 space-y-4 text-[17px] leading-relaxed text-muted sm:text-[19px]">
                {about.details.split(/\n\n+/).map((para, i) => (
                  <p key={i}>{para}</p>
                ))}
              </div>
